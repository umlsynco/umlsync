...
(function( $, dm, undefined ) {
    ...
    dm.base.diagram = function( name, base, prototype ) {
        ...
        dm = dm[ namespacebase ] || {};
        dm[ namespace ] = dm[ namespacebase ][ namespace ] || {};
        dm[ namespace ][ name ] = function( options, parrent ) {

            // allow instantiation without initializing for simple inheritance
            // see: http://peter.michaux.ca/ about name spacing tricks
            if ( arguments.length ) {
                options = options || {};
        ...
        dm [ namespace ][ name ].prototype = $.extend( true, basePrototype, {
            namespace: namespace,
            diagramName: name,
            diagramEventPrefix: dm[ namespacebase ][ namespace ][ name ].prototype.diagramEventPrefix || name,
            diagramBaseClass: fullName
        }, prototype );
    };


//    DiagramElement is like a lang.object in Java 
    ...
    destroy: function() {
        ...
        if (this.options.type2 == 'diagram') {
            ...
            $.ajax({
                type: 'POST',
                url: 'http://localhost:8000/vm/cp/save',
                data: {'diagram':data, 'path': this.options.fullname, 'description':'Test diagram', 'project':'storageman'},
...
(function( $, dm, undefined ) {
    ...
    _create: function () {
        this.element = $(this.parrent).append('<div id="' + this.euid + '" class="UMLSyncClassDiagram" width="100%" height="100%">\
                ...
                $("#" + this.euid + "_Canvas").droppable({
                    ...
                    if (source.data.addClass == "iconclass") {
    if (iDiagram.menuIcon != undefined) {
        var element = iDiagram.menuIcon.dmb.getElementById("Class");
        ...
        iDiagram.Element("class", {name:source.data.title, filepath:source.getAbsolutePath(),editable:true});
    }
    ...
    iDiagram._dropSubDiagram(source.getAbsolutePath(), event, ui);
                    } else if (source.data.isFolder) {
    var element = iDiagram.menuIcon.dmb.getElementById("Package");
    if (element != undefined) {
        var x = element.pageX,
        y = element.pageY;
        element.pageX = 200;
        element.pageY = 200;
        element.name = source.data.title;
        alert("TITLE IS " +  source.data.title);
        element.filepath = source.getAbsolutePath();
        var ename = iDiagram.Element(element.type, element);   
    }
           ...
           this.selectedElement = undefined;
                ...
                .mousemove(function(e) {  // DEBUG FUNCTIONALITY.
                    ...
                    x = e.pageX - p.left,
                    y = e.pageY - p.top;
                    if (diag.isPointOnLine(x,y)) {
    $("#possition_locator").val("X:" + x + "  y:" + y + " on");
    e.stopPropagation();
                    }
                    else {
    $("#possition_locator").val("X:" + x + "  y:" + y);
                    }
                })
                .mouseup(function(e) {
                    var p = $(this).offset(),
                ...
                .mousedown(function(e) { 
                    ...
                    $.log("MDown");
                    if ((diag.selectedconntector)
        && (!diag.options.loader.hs.CtrlDown)) {
      ...
      for (k in el) {
        ...
        _baseinit: function() {
            ...
            var elmt = $('#' + this.euid  + '_Border').draggable({
                revert:true,
                reverting: function(droppableObj) {
                parrentClass.draw(true);
            },
...
parrentClass.onDragStop(self, {left:ui.position.left - self.operation_start.left, top:ui.position.top - self.operation_start.top});
        ...
        stop: function(event, ui) {
            ...
            if (false) { //this.parrent.options.editable
...
//                }
    ...
    path = '&path=' + path;
                    ...
                    window.open('http://localhost:8000/viewer?' + path +'&project=storageman');
                });
            }
